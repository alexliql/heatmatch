...
from ingest.sources import nova_mw
...
def one_row(monkeypatch):
    def use(rows):
        monkeypatch.setattr(nova_mw, "_rows", lambda: rows)
...
def test_a_curated_row_records_its_campus_and_its_source(one_row) -> None:
    ...
    assert nova_mw.NOVA_MW_SOURCE["id"] in dcs[0]["sources"]
...
def test_an_unmatched_row_is_counted_not_silently_dropped(one_row) -> None:
    ...
    stats = nova_mw.apply([_dc("QTS Manassas DC1", 30.0)])
...
def test_every_shipped_row_carries_a_source_url() -> None:
    ...
    for row in nova_mw._rows():
        assert row["source_url"].startswith("http"), row["match_key"]
        assert row["source_date"], row["match_key"]
        assert row["mw"] > 0, row["match_key"]
        assert row["mw_confidence"] in ("reported", "filed"), row["match_key"]
