...
def test_join_replaces_the_footprint_guess(monkeypatch) -> None:
    monkeypatch.setattr(
        ll84, "by_bbl", lambda refresh=False: {"1234": {"demand_kwh": 500_000.0, "steam_heated": False}}
    )
    rows = [{"lat": 40.7000, "lon": -74.0000, "demand_kwh": 1.0,
             "demand_source": "footprint_estimate", "steam_heated": False}]
...
def test_steam_heated_buildings_are_flagged_not_rewritten(monkeypatch) -> None:
    ...
    rows = [{"lat": 40.7, "lon": -74.0, "demand_kwh": 1.0,
             "demand_source": "footprint_estimate", "steam_heated": False}]
    lots = [{"bbl": "1234", "latitude": "40.70001", "longitude": "-74.00001"}]
